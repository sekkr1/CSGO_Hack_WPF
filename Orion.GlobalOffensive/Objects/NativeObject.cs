...
namespace Orion.GlobalOffensive.Objects
{
	...
	public abstract class NativeObject : IEquatable<NativeObject>
	{
        ...
        /// <summary>
		///     Writes a member of the specified type at the specified offset.
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <param name="offset">The offset.</param>
		/// <returns></returns>
		protected void WriteField<T>(int offset,T value) where T : struct
        {
            // We don't check for IsValid here because reading even on an invalid object would
            // not directly be an invalid operation - the read operation will throw an exception later down the line.

            Orion.Memory.Write<T>(BaseAddress + offset,value);
        }
        ...
        /// <param name="offset">The offset.</param>
        /// <returns></returns>
        protected void WriteField<T>(StaticOffsets offset, T value) where T : struct
        {
            WriteField<T>((int)offset, value);
        }
